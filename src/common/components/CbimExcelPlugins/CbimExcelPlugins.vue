      ...
      <Table :columns="columns" :no-data-text="indexNoDataTextFn"  :data="excelDatas" style="width: 100%">
        ...
        <template v-for="(item,index) in columns" slot-scope="{ row,column,index }" :slot="item.slot">
          ...
          <div v-else>
            ...
            <Tooltip v-if="!row[item.slot].flag" :content="row[item.slot].errorInfo" placement="top" style="width: 100%;">
    ...
    export default {
        ...
        methods: {
          ...
          downloadExcel(type){
            ...
            let exportDaras = this.getExcelContents('dowload').exportDatas;
          ...
          download(Ary,type){
            ...
            try{
              ...
              const wopts = { bookType:'csv', bookSST:true, type:'binary'};
              ...
              this.saveAs(tmpDown,`${this.fileName.substring(0,this.fileName.lastIndexOf("."))}.${type}`)
